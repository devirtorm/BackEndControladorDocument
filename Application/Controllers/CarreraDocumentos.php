...
class ControllersCarreraDocumentos extends Controller
{
    ...
    public function ActualizarCarreraDocumento()
    {
        $segments = explode('/', rtrim($_SERVER['REQUEST_URI'], '/'));
        $id = end($segments);
        $id = intval($id);
        ...
        if ($data !== null && isset($data['fk_carrera']) && isset($data['fk_documento']) && isset($data['tsu']) && isset($data['ing'])) {
            $fk_carrera = filter_var($data['fk_carrera'], FILTER_VALIDATE_INT);
            $fk_documento = filter_var($data['fk_documento'], FILTER_VALIDATE_INT);
            $tsu = filter_var($data['tsu'], FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE);
            $ing = filter_var($data['ing'], FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE);

            if ($fk_carrera !== false && $fk_documento !== false && $tsu !== null && $ing !== null) {
                $updated = $model->updateCarreraDocumento($id, $fk_carrera, $fk_documento, $tsu, $ing);
                ...
                echo json_encode(['message' => 'Error: Los datos son inválidos.']);
            }
        } else {
